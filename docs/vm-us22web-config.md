...
- la fiche technique de création d'une VM sous VMWare à partir d'un modèle, [ici](https://siovhblyceebasch-my.sharepoint.com/personal/vhbsio_lycee-basch_fr/Documents/VHBSIO/_Etudiants/_SIO_Commun_Ressources/DocumentationTechnique/FermeServeurs_VMWare/FT_esx_vmware_modele_creerMV.docx),
...
- Choisir l’emplacement `VM_ELEVES/SIO2_2425/SLAM` pour le stockage de la nouvelle VM
...
- Passer la configuration réseau IP de votre serveur US22_WEB en mode statique sur l'adresse IP 100.115.29.x / 23, x étant le numéro de la 2ème adresse IP du groupe de 5 adresses IP qui vous a été attribué. Les groupes sont notés dans le fichier `20232025_ListegestionStage.xls` sous Triskell. _Ne pas oublier la configuration de la passerelle et du serveur DNS._
...
### Installer la version PHP8.2 et les extensions nécessaires pour Symfony
- Demander à voir la version courante l'interpréteur php.
- Comme la version 7 de Symfony impose la version 8.2 de Symfony, suivons la procédure suivante :
```bash
sudo add-apt-repository ppa:ondrej/php
sudo apt update
sudo apt install php8.2 php8.2-cli php8.2-{bz2,curl,mbstring,intl,xml,mysql}
```
- Vérifier que les extensions `bz2` à `mysql` ont bien été installées par la commande :
  ```bash
  php -m 
  ```
...
- Y créer un nouveau jeton nommé NomEtudiant_Prod ayant le rôle Reporter et la permission read_repository

Ce jeton sera ensuite utilisé dans une URL de type https ainsi formée :
`https://gitlab-ci-token:<votrejeton>@gitlab.siovhb.lycee-basch.fr:8443/votreEspaceNom/votredepot.git`
 
### Récupérer votre branche du dépôt gitlab de groupe
De manière identique à la récupération du dépôt sous xampp de W10, vous allez "descendre" votre branche du dépôt sous le répertoire de publication du site web par défaut d'apache2.
...
```
- Affecter également la valeur de la variable `APP_ENV` à la valeur `prod`.
...
- Sur le serveur us22_web lui-même, vérifier par la commande `curl` le bon accès à l'API-REST pour obtenir la liste complète de vos ressources, organisations, contacts ou stages.
- Sur la machine hôte, invoquer l'API-REST sous votre navigateur favori. 
- Sous Talend API-Tester, créer un nouvel environnement nommé `GestionStages-prod` correspondant à votre serveur de recette, et y retester vos requêtes http opérationnelles sous votre environnement de développement.
...
- Vérifier le contenu des logs de Symfony `./var/log/prod.log`
